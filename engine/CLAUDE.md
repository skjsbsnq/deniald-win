...
跨平台输入法，Core 平台无关，各平台只做壳。已发版 macOS 0.1.2（自用 + 测试者）、Windows 0.1.0-alpha.2（内测）；Linux 壳开工中（`apps/linux`，常驻引擎服务进程）。
...
- `apps/linux`：常驻引擎服务进程 `qingjian-linux`：装配 Engine，作为 input-engine-v1 客户端连宿主合成器的 `$XDG_RUNTIME_DIR/denial/ime.sock`（SOCK_SEQPACKET）；`mock_deniald` bin 是回放协议的联调工具。
