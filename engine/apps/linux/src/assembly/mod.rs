//! 装配 Engine：Linux 壳里唯一知道具体 Translator / Learner 类型的地方，
//! 装的东西与 Windows Server 的 `assembly` / macOS 的 `host::init` 一致。
...
/// 按 [`AssemblySpec`] 装配 Engine：主词库必装，其余缺哪个少哪个功能。
pub fn assemble(spec: &AssemblySpec) -> Result<Engine, LinuxError> {
    let started = Instant::now();
    let dictionary =
        Dictionary::from_path(&spec.dict).map_err(|source| LinuxError::Dictionary {
            path: spec.dict.clone(),
            source,
        })?;
    // 学习数据 / 输入日志 / 用户词库都在用户目录下，先把它建出来。
    if let Some(dir) = &spec.user_dir
        && let Err(error) = std::fs::create_dir_all(dir)
    {
        tracing::warn!(%error, dir = %dir.display(), "建用户数据目录失败，本次只在内存里学");
    }
    ...
    if let Some(path) = &spec.english {
        let words = WordList::from_path(path).map_err(|source| LinuxError::WordList {
            path: path.clone(),
            source,
        })?;
...
) -> Result<LayeredTranslator, LinuxError> {
    let bundled = Glossary::from_path(language, path).map_err(|source| LinuxError::Glossary {
        path: path.to_owned(),
        source,
    })?;
