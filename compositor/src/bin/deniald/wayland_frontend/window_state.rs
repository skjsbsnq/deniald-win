...
impl WaylandFrontend {
    ...
    /// Stable identity of the window this one is transient for, if any.
    ///
    /// Workspace membership is inherited through this relationship so a dialog
    /// follows the monitor-local workspace of its parent instead of joining
    /// whatever workspace happens to be visible when it maps.
    ...
    /// Minimizes or restores a local Flutter window while keeping its
    /// workspace membership in sync with the minimization state.
    #[cfg(feature = "flutter")]
