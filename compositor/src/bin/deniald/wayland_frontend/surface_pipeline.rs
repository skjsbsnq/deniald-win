...
impl WaylandFrontend {
    ...
    pub fn flutter_scene(
        ...
        // Workspace reconciliation mutates the frontend's ownership maps below,
        // so the scene list cannot hold a live borrow of `self.space`.
        ...
        for window in &scene_windows {
            ...
            let monitor_id = self
                ...
                .unwrap_or(-1);
            // Monitor-local workspace ownership. A transient window without an
            // assignment of its own inherits its parent's membership so dialogs
            // never vanish with the workspace of a different monitor.
        ...
        // Local windows reconcile workspace ownership below, so iterate a
        // snapshot instead of holding a borrow of `self.local_windows`.
        ...
        if let Some(cursor_rectangle) = input_method_editor_rectangle {
            for popup in input_method_popups {
                ...
                let description = WindowDescription {
                    ...
                    geometry_height: f64::from(geometry.size.h),
                    monitor_id,
                    // Software-keyboard surfaces are compositor chrome, not
                    // workspace members.
                    workspace_id: 1,
                    minimized: false,
                    transform,
