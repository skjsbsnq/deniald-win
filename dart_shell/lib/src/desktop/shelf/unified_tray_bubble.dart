...
class _UnifiedTrayBubbleState extends ConsumerState<UnifiedTrayBubble>
  ...
  // burst, short enough that the first tray click lands on an inflated tree.
  ...
  /// inflates while the bubble is still invisible.
  void _runWarmup() {
    _warmupTimer = null;
    if (!mounted || _hasInflated) {
      return;
    ...
    return AnimatedBuilder(
      ...
      builder: (context, child) {
        ...
        if (hidden && !_hasInflated) {
          ...
          // startup does not pay the bubble's inflate. Once inflated, the
